...
interface TransactionItemProps {
  ...
  index: number;
  onClick: () => void; // 1. Added onClick prop
}

const TransactionItem: FC<TransactionItemProps> = ({ transaction, index, onClick }) => {
  const isIncome = transaction.type === 'income';

  return (
    <motion.div 
      onClick={onClick} // 2. Trigger it here
      initial={{ opacity: 0, x: -20 }}
      animate={{ opacity: 1, x: 0 }}
      exit={{ opacity: 0, scale: 0.95 }}
      transition={{ delay: index * 0.05 }}
      // 3. Added 'cursor-pointer' to the classes
      className="flex items-center justify-between p-4 bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl hover:bg-black/60 transition cursor-pointer"
    >
