...
import {
  Plus, Wallet, PiggyBank, Receipt, Calendar, ChevronRight, LogOut, LogIn, Loader2, ShieldAlert,
} from "lucide-react";
...
export default function ExpensesApp() {
  ...
  // --- PASTEL PINK & GOLD THEME ---
  const isHerTheme = user?.email?.toLowerCase() === "tigermeow26@gmail.com";
  const theme = {
      textPrimary: isHerTheme ? "text-pink-200" : "text-blue-400",
      textSecondary: isHerTheme ? "text-amber-200" : "text-blue-400", 
      bgSubtle: isHerTheme ? "bg-pink-200/20" : "bg-blue-500/20",
      borderSubtle: isHerTheme ? "border-pink-200/50" : "border-blue-500/50",
      fabBg: isHerTheme 
        ? "bg-gradient-to-br from-pink-200 to-amber-200 hover:from-pink-300 hover:to-amber-300 text-gray-900" 
        : "bg-blue-500 hover:bg-blue-600 text-white",
      fabShadow: isHerTheme 
        ? "shadow-[0_0_20px_rgba(251,207,232,0.4)]" 
        : "shadow-[0_0_20px_rgba(59,130,246,0.5)]",
      // CHANGED: Pointing directly to your local file
      bgImage: isHerTheme
        ? "url('pattern.png')" // (Change to .jpg if your file is actually a jpeg!)
        : "url('https://images.unsplash.com/photo-1557683316-973673baf926?q=80&w=2000&auto=format&fit=crop')",
  };

  useEffect(() => {
    if (!user || !user.email || !ALLOWED_EMAILS.includes(user.email.toLowerCase())) return;
    ...
    const unsubPockets = onSnapshot(qPockets, (snapshot) => {
      ...
      snapshot.forEach((doc) => pocketData.push({ id: doc.id, ...doc.data() } as Pocket));
      setPockets(pocketData);
    });

    return () => { unsubTxs(); unsubPockets(); };
  }, [user]);
  ...
  const { groupedTransactions, months } = useMemo(() => {
    ...
    const groups = sorted.reduce((acc, tx) => {
      const monthYear = new Date(tx.date).toLocaleString("en-GB", { month: "long", year: "numeric" });
  ...
  if (loadingAuth) {
    return (
      <div className="min-h-[100dvh] bg-gray-950 flex flex-col items-center justify-center text-white">
        <Loader2 className={`animate-spin mb-4 ${theme.textPrimary}`} size={48} />
        <p className="text-gray-400 font-medium tracking-wide">Verifying secure vault...</p>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div className="relative min-h-[100dvh] text-white font-sans bg-gray-950 flex items-center justify-center p-4">
        <div className="absolute inset-0 z-0 bg-cover bg-center opacity-40 mix-blend-overlay transition-all duration-1000" style={{ backgroundImage: theme.bgImage }} />
        <div className="relative z-10 w-full max-w-sm bg-black/40 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl text-center">
          ...
          <p className="text-gray-400 text-sm mb-8">Sign in to access your shared financial dashboard.</p>
          <button onClick={handleLogin} className="w-full bg-white text-gray-900 hover:bg-gray-200 font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-3 transition-colors">
            <LogIn size={20} /> <span>Sign in with Google</span>
          </button>
        </div>
      </div>
    );
  ...
  if (user.email && !ALLOWED_EMAILS.includes(user.email.toLowerCase())) {
    return (
      <div className="relative min-h-[100dvh] text-white font-sans bg-gray-950 flex items-center justify-center p-4">
        <div className="absolute inset-0 z-0 bg-cover bg-center opacity-20 mix-blend-overlay grayscale transition-all duration-1000" style={{ backgroundImage: theme.bgImage }} />
        <div className="relative z-10 w-full max-w-sm bg-black/60 backdrop-blur-xl border border-red-500/30 p-8 rounded-3xl shadow-2xl text-center">
          ...
          <p className="text-gray-400 text-sm mb-2">The account <span className="text-gray-200 font-semibold">{user.email}</span> is not authorized.</p>
          <p className="text-gray-500 text-xs mb-8">This is a private dashboard.</p>
          <button onClick={() => signOut(auth)} className="w-full bg-white/10 text-white hover:bg-red-500/20 hover:text-red-300 font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-3 transition-colors border border-white/5 hover:border-red-500/30">
  ...
  return (
    <div className={`relative min-h-[100dvh] text-white font-sans ${isHerTheme ? 'bg-black' : 'bg-gray-950'} overflow-y-auto pb-24`}>
      {/* BACKGROUND LAYER */}
      <div
        className={`fixed inset-0 z-0 transition-all duration-1000 ${
          isHerTheme 
            ? 'opacity-50 bg-repeat bg-[length:600px]' 
            
            // Your default blue theme keeps its full-screen stretched image
            : 'opacity-40 mix-blend-overlay bg-cover bg-center'
        }`}
      ...
      <div className="relative z-10 w-full max-w-[95%] xl:max-w-[1400px] mx-auto px-4 sm:px-6 pt-6 lg:pt-12 flex flex-col lg:flex-row gap-6 lg:gap-10">
        <aside className="w-full lg:w-72 xl:w-80 flex-shrink-0">
          <div className="lg:sticky lg:top-12">
            <h2 className="text-3xl font-bold tracking-tight text-white drop-shadow-lg mb-6 hidden lg:block">Our Finances</h2>
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 ml-2">Timeline</h3>
            <div className="flex lg:flex-col gap-2 overflow-x-auto pb-2 lg:pb-0 scrollbar-thin snap-x [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {months.map((month) => (
                ...
                >
                  ...
                  {selectedMonth === month && <ChevronRight size={16} className={`hidden lg:block ${theme.textPrimary}`} />}
                </button>
              ))}
            </div>
          </div>
        ...
        <main className="flex-1 min-w-0">
          <header className="flex justify-between items-center mb-6 lg:hidden">
            <div><h1 className="text-3xl md:text-4xl font-bold tracking-tight text-white drop-shadow-lg">Our Finances</h1></div>
            <button onClick={() => signOut(auth)} className="bg-white/10 hover:bg-red-500/20 hover:text-red-400 hover:border-red-500/50 p-2 rounded-full backdrop-blur-md border border-white/10 transition">
              <LogOut size={24} />
            </button>
          </header>
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-2 text-xl font-semibold">
              <Calendar size={24} className={theme.textSecondary} />
              <h2>{selectedMonth} Overview</h2>
            </div>
            <button onClick={() => signOut(auth)} className="hidden lg:flex items-center gap-2 bg-white/10 hover:bg-red-500/20 hover:text-red-400 hover:border-red-500/50 px-4 py-2 rounded-xl backdrop-blur-md border border-white/10 transition text-sm font-medium">
              <LogOut size={16} /> Sign Out
            </button>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8 lg:mb-10">
            <OverviewCard title="Net Balance" amount={monthBalance} type="neutral" />
            <OverviewCard title="Income" amount={monthIncome} type="income" />
            <OverviewCard title="Expenses" amount={monthExpense} type="expense" />
          </div>
          ...
          <section className="mb-10">
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-thin snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              ...
              <button
                ...
                className={`flex-shrink-0 w-40 h-32 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center transition snap-center cursor-pointer ${
                  isHerTheme 
                    ? 'border-rose-300/50 text-rose-900/50 hover:text-rose-950 hover:bg-white/40 hover:border-rose-400 bg-white/20'
          ...
          <section>
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
              <Receipt size={20} className={theme.textSecondary} /> Activity
            </h2>
              ...
              <AnimatePresence mode="popLayout">
                {currentMonthTxs.length > 0 ? (
                 currentMonthTxs.map((tx, i) => (
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-10 bg-black/20 backdrop-blur-md rounded-3xl border border-white/5">
                    <p className="text-gray-400">No transactions recorded for {selectedMonth}.</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 lg:bottom-10 lg:right-10 z-50 p-4 lg:p-5 rounded-full transition-transform hover:scale-105 ${theme.fabBg} ${theme.fabShadow}`}
      >
